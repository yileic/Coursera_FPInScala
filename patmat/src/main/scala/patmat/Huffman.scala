...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    chars.groupBy(x => x).map(t => (t._1, t._2.length)).toList
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] =
    trees match {
      case first :: second :: xs => (makeCodeTree(first, second) :: xs).sortBy(weight)
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    table.filter(list => list._1 == char).head._2
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def prepend(bit: Bit)(code: Code): Code =
      (code._1, bit :: code._2)
