...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  // ((left union that) union right).incl(elem) -- this will timeout

  def mostRetweeted: Tweet = {
    def getLargerRetweets(a: Tweet, b: Tweet): Tweet = {
    ...
    if (left.isEmpty && right.isEmpty) {
      ...
      getLargerRetweets(elem, right.mostRetweeted)
    } else if (!left.isEmpty && right.isEmpty) {
      getLargerRetweets(elem, left.mostRetweeted)
    } else {
      getLargerRetweets(getLargerRetweets(left.mostRetweeted, right.mostRetweeted), elem)
    }
  }
